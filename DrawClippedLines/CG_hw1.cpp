...
void writePixel(int x, int y, int sign, int index, int length, double scal, int rota, int xTran, int yTran);
...
int main(int argc, char* argv[])
{
	...
	int xyOpIndex = 0;  //Index for x,y-coop
...
void bresenham(int qx, int qy, int rx, int ry, int sign, int index, double scal, int rota, int xTran, int yTran)
{
	...
	for (x = qx; x <= rx; x++)
	{
		writePixel(x, y, sign, index, length_line, scal, rota, xTran, yTran);
		//cout << x << "," << y << endl;
		if(D <= 0)
		{
			D += 2 * dy;
...
void writePixel(int x, int y, int sign, int index, int length, double scal, int rota, int xTran, int yTran)
{
	//Mapping various types of slope of points and doing corresponding work
	double tempX = x * scal;
	double tempY = y * scal;
	...
	switch (sign)
	{
		...
		case 1:
			...
			break;
		case 2:	
			xOpPath[index][length] = tempY * cosValue - tempX * sinValue + xTran;
			yOpPath[index][length] = tempY * sinValue + tempX * cosValue + yTran;
